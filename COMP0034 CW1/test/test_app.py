...
def test_cycle013_monthly_avgusage_graph_title_equals(dash_duo, app):
    ...
    THEN the title should include the text "Average Cycle Hire Usage per Month" (not case sensitive)
    """
    dash_duo.driver.implicitly_wait(10)
    title = dash_duo.find_element("#monthly-avgusage-graph .gtitle").text
    assert title.casefold() == "Average Cycle Hire Usage per Month".casefold()

def test_cycle014_monthly_totusage_graph_title_equals(dash_duo, app):
    """
    GIVEN the Dash app is running
    WHEN the Monthly total usage figure has loaded
    THEN the title should include the text "Cycle Hire Usage per Month across 11 Years" (not case sensitive)
    """
    dash_duo.driver.implicitly_wait(10)
    title = dash_duo.find_element("#monthly-totusage-graph .gtitle").text
    assert title.casefold() == "Cycle Hire Usage per Month across 11 Years".casefold()

def test_cycle015_pollution_bar_graph_title_equals(dash_duo, app):
    """
    GIVEN the Dash app is running
    WHEN the Pollution bar chart has loaded
    THEN the title should include the text "Pollution Levels in the Boroughs of London" (not case sensitive)
    """
    dash_duo.driver.implicitly_wait(10)
    title = dash_duo.find_element("#pollution-bar-graph .gtitle").text
    assert title.casefold() == "Pollution Levels in the Boroughs of London".casefold()
